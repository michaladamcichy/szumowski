...
public:
	...
	Config() {
		...
		ints[FRAME_DELAY] = 0;
		ints[TIME_MEASUREMENT_FREQUENCY] = 200;
		...
		bools[HD_TEXTURES_ENABLED] = true;

		vec3s[GROUND_DIMENSIONS] = vec3(1000); //ALERT ZROBILEM UNIFORMOWE SKALOWANIE, ZEBY NIE PSUC NORMALNYCH. ALE CHYBA JEST OK
		vec3s[PLAYER_INITIAL_POSITION] = vec3(0, 0, 0);
		vec3s[VIRUS_DIMENSIONS] = vec3(1.0, 1.0, 1.0);
		vec3s[DEFAULT_BUILDING_DIMENSIONS] = vec3(10.0, 40.0, 10.0);
		vec3s[SKY_COLOR] = vec3(0.125, 0.72, 0.85);
